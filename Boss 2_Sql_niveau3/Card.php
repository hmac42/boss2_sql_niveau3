...
class Card
{
	...
	public function __construct($id, $values=null)
	{
		...
		if (isset($values['nom']))
		{
			$this->nom = $values['nom']; // TODO à faire sur le même modèle que précédemment
		}
		if (isset($values['prix']))
		{
			$this->prix = $values['prix']; // TODO à faire
		}
		if (isset($values['image']))
		{
			$this->image = $values['image']; // TODO à faire
		}
		if (isset($values['url']))
		{
	...
	public static function getAllCards($sort="pd")
	{
		...
		$values = self::$database->get("cartes","*",["ORDER"=>$array_sort]);// à compléter, voir doc de Medoo : https://medoo.in/api/select
		$results = array() ;
		for($i = 0; $i < sizeof($values); ++$i) {
			$carte = new Card($values['id'], $values[$i]) ;// TODO : à compléter pour que l'appel au constructeur Card construise bien chaque carte avec les bons paramètres
	...
	public static function getJSONNames()
	{
		...
		$values = self::$database->get("cartes",'*'); // à compléter, voir doc de Medoo : https://medoo.in/api/select
		$json_array = array() ;
		for($i = 0; $i < sizeof($values); ++$i) {
			$carte = new Card($values['id'], $values[$i]) ; // TODO : à compléter pour que l'appel au constructeur Card construise bien chaque carte avec les bons paramètres (regardez la signature du constructeur)
